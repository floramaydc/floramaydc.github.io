import{E as L,a as w}from"./jspdf.plugin.autotable.BNWpDLH5.js";import"./preload-helper.BlTxHScW.js";function s(n){return String(n).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function k(n,e,i){const m=s(e),p=i?s(i):"";n.innerHTML=`
  ...
  `}function U(n,e){const i=String(e||"").toLowerCase();return n===429?"The service is currently busy. Please retry in about 30-60 seconds.":n===502||n===503||n===504?"The audit service is temporarily unavailable or under maintenance. Please try again shortly.":n===408?"The page took too long to load. Retry once, then test a lighter page to confirm connectivity.":n===422||i.includes("could not reach")||i.includes("could not be resolved")?"Verify that the page is public, not login-gated, and not blocking automated browsers.":"Please try again in a moment. If this keeps happening, the audit service may be temporarily unavailable."}function P(n){const e=["Connecting to M.ai audit service","Opening page in a secure browser session","Inspecting component inventory and risks","Packaging your report"];let i=0;const m=()=>{const u=e.map((x,r)=>{const c=r===i;return`<li class="flex items-center gap-2 text-sm ${c?"text-ink font-semibold":"text-muted"}"><span class="inline-block h-2 w-2 rounded-full ${c?"bg-accent animate-pulse":"bg-ink/20"}" aria-hidden="true"></span>${s(x)}</li>`}).join("");n.innerHTML=`
    ...
    `};m();const p=window.setInterval(()=>{i=(i+1)%e.length,m()},1800);return{stop(){window.clearInterval(p)}}}function A(n){const e=String(n||"").trim();if(!e)return"The audit request failed.";const i=e.replace(/^component audit failed:\s*/i,""),p=i.split(/browser logs?:/i)[0].trim()||i,u=p.toLowerCase();return u.includes("target page, context or browser has been closed")?"This site could not be audited because the browser session closed unexpectedly.":u.includes("timeout")||u.includes("timed out")?"The audit timed out before the page finished loading.":u.includes("err_name_not_resolved")||u.includes("dns")?"The URL could not be resolved.":u.includes("net::err_")||u.includes("failed to fetch")?"The audit could not reach that URL.":p.length>220?`${p.slice(0,217)}...`:p}function C(n,e=10){return(Array.isArray(n?.findings)?n.findings:[]).slice(0,e).map((m,p)=>({...m,annotationId:m?.annotationId??p+1}))}function I(n){return n?`${n.x}, ${n.y}, ${n.width} × ${n.height}`:""}function F(n,e,i){return n.filter(m=>m?.boundingBox).map(m=>{const p=m.boundingBox,u=p.x/e*100,x=p.y/i*100,r=p.width/e*100,c=p.height/i*100;return`
      ...
      `}).join("")}function T(n){return n==="high"?"text-red-700":n==="medium"?"text-orange-600":"text-emerald-700"}function R(n){return n==="high"?"bg-red-50 text-red-700 border border-red-200":n==="medium"?"bg-orange-50 text-orange-700 border border-orange-200":"bg-emerald-50 text-emerald-700 border border-emerald-200"}function M(n){return n==="high"?"border-2 border-red-500/90 bg-red-500/10":n==="medium"?"border-2 border-orange-400/90 bg-orange-400/10":"border border-emerald-400/60"}function j(n){const e=n?.summary||{},i=C(n,10),m=Array.isArray(n?.inventory)?n.inventory:[],p=n?.fluentMeta||{},u=n?.notice,x=Object.entries(e.componentDistribution||{}).map(([t,l])=>`  - ${t}: ${l}`).join(`
`),r=["# Component Audit","",`- URL: ${n?.url||""}`,`- Library context: ${n?.libraryContext||"Generic"}`,`- Timestamp: ${n?.timestamp||""}`,`- Total components: ${e.totalComponents??0}`,`- Custom: ${e.customComponentCount??0}  Native: ${e.nativeComponentCount??0}  Carbon: ${e.carbonComponentCount??0}  Fluent UI: ${e.fluentComponentCount??0}  Material UI: ${e.materialComponentCount??0}`,`- Risk — High: ${e.riskSummary?.high??0}  Medium: ${e.riskSummary?.medium??0}  Low: ${e.riskSummary?.low??0}`];if(u&&r.push("",`> **Note:** ${u}`),p?.detected){const t=p.versionDetected?`\`${p.versionDetected}\` detected${p.versionLatest?` — latest is \`${p.versionLatest}\``:""}${p.isOutdated?" (outdated)":""}`:"version not pinned in script URLs";r.push("","## Fluent fingerprint","",`- \`fui-*\` elements on page: ${p.totalFuiElements??0}`,`- @fluentui/react-components: ${t}`,`- Storybook: ${p.storybookHome||"https://react.fluentui.dev"}`)}r.push("","## Component distribution","",x||"  (none)","","## Method","","- Observe: visit the page, capture screenshot, extract DOM inventory.","- Inventory: build a typed component list with source detection and confidence scores.","- Annotate: apply per-library rule packs (Carbon / Fluent UI / Material UI) to flag risky implementations.","- Recommend: map each finding to the appropriate library component with explanation. Pages with no design system get generic accessibility guidance.","","## Annotations",""),i.length===0?r.push("No risky implementations found.",""):i.forEach(t=>{const l=t.detectedProps&&Object.keys(t.detectedProps).length?Object.entries(t.detectedProps).map(([a,o])=>`${a}=${o}`).join(", "):"—";r.push(`### Annotation ${t.annotationId} — ${t.rule||"Finding"}`,"",`- Risk: ${t.severity}${t.priority?` (${t.priority})`:""}`,`- Component type: ${t.componentType}`,`- Fluent component: ${t.fluentComponent?`\`${t.fluentComponent}\``:"—"}`,`- Storybook: ${t.storybookUrl||"—"}`,`- Detected props: ${l}`,`- Selector: ${t.selector||""}`,`- Bounding box: ${I(t.boundingBox)}`,`- Evidence: ${t.evidence||""}`,`- Recommendation: ${t.recommendation||""}`,`- Library: ${t.libraryLabel||"none detected"}`,`- Suggested component: ${t.recommendedComponent||t.fluentAlternative||"n/a"}`,`- Package: ${t.recommendedPackage||t.fluentPackage||"n/a"}`,`- Benefits: ${(t.benefits||[]).join(", ")}`,`- Explanation: ${t.explanation||""}`,"")}),r.push("## Findings",""),i.length===0?r.push("No issues found."):(r.push("| # | Risk | Rule | Type | Selector | Library | Suggested component |","|---|---|---|---|---|---|---|"),i.forEach(t=>{r.push(`| ${t.annotationId} | ${t.severity} | ${t.rule} | ${t.componentType} | ${t.selector||""} | ${t.libraryLabel||"—"} | ${t.recommendedComponent||t.fluentAlternative||"n/a"} |`)})),r.push("","## Screenshot","");const c=n?.screenshot;return c?.data?r.push(`![Component audit screenshot](data:${c.mimeType||"image/png"};base64,${c.data})`):r.push("Screenshot unavailable."),r.push("","## Component Inventory","","| ID | Type | Library component | Props | Source | Risk | Conf. | Text | Selector | Docs |","|---|---|---|---|---|---|---|---|---|---|"),m.forEach(t=>{const l=t?.detectedProps&&Object.keys(t.detectedProps).length?Object.entries(t.detectedProps).map(([g,h])=>`${g}=${h}`).join(" "):"—",a=t?.storybookUrl?`[docs](${t.storybookUrl})`:"—",o=String(t?.visibleText||"").replace(/\|/g,"\\|"),b=String(t?.selector||"").replace(/\|/g,"\\|");r.push(`| ${t?.id||""} | ${t?.componentType||""} | ${t?.libraryComponentName||t?.fluentComponent||"—"} | ${l} | ${t?.possibleSource||""} | ${t?.risk||""} | ${t?.confidence??""}% | ${o} | ${b} | ${a} |`)}),r.push("","## M.ai Context","","```json",JSON.stringify(n?.maiContext||{},null,2),"```"),r.join(`
`)}function D(n){const e=new L({orientation:"landscape",unit:"pt",format:"letter"}),i=n?.summary||{},m=C(n,10),p=Array.isArray(n?.inventory)?n.inventory:[],u=n?.screenshot,x=`component-audit-${new URL(n.url).hostname}-${new Date(n.timestamp||Date.now()).toISOString().slice(0,10)}`,r=e.internal.pageSize.getWidth(),c=40,t=r-c*2;let l=48;if(e.setFont("helvetica","bold"),e.setFontSize(18),e.text("Component Audit by M.ai",c,l),l+=18,e.setFont("helvetica","normal"),e.setFontSize(10),e.text(`URL: ${n.url}`,c,l),l+=14,e.text(`Library context: ${n.libraryContext||"No design system detected"}`,c,l),l+=14,e.text(`Timestamp: ${n.timestamp||""}`,c,l),l+=18,w(e,{startY:l,margin:{left:c,right:c},head:[["Total","Custom","Native","Carbon","Fluent UI","Material UI","High Risk","Medium Risk"]],body:[[String(i.totalComponents??0),String(i.customComponentCount??0),String(i.nativeComponentCount??0),String(i.carbonComponentCount??0),String(i.fluentComponentCount??0),String(i.materialComponentCount??0),String(i.riskSummary?.high??0),String(i.riskSummary?.medium??0)]],styles:{fontSize:9,cellPadding:4},headStyles:{fillColor:[40,40,40]}}),l=e.lastAutoTable.finalY+18,u?.data){e.setFont("helvetica","bold"),e.setFontSize(14),e.text("Screenshot with annotations",c,l),l+=10;const a=Number(u.width||1),o=Number(u.height||1),b=Math.max(220,Math.round(t*(o/a)));e.addImage(`data:${u.mimeType||"image/png"};base64,${u.data}`,"PNG",c,l,t,b);const g=t/a,h=b/o;m.forEach(d=>{if(!d.boundingBox)return;const y=c+d.boundingBox.x*g,f=l+d.boundingBox.y*h,$=d.boundingBox.width*g,S=d.boundingBox.height*h;if($>0&&S>0){const v=d.severity==="high";e.setDrawColor(v?204:234,v?51:88,v?51:12),e.setLineWidth(1),e.rect(y,f,$,S),e.setFillColor(v?122:160,v?40:80,v?40:10),e.circle(y+8,f+8,8,"F"),e.setTextColor(255,255,255),e.setFont("helvetica","bold"),e.setFontSize(8),e.text(String(d.annotationId),y+8,f+10.5,{align:"center"}),e.setTextColor(0,0,0)}}),l+=b+18}e.setFont("helvetica","bold"),e.setFontSize(14),e.text("Annotations",c,l),l+=8,m.length===0?(e.setFont("helvetica","normal"),e.setFontSize(10),e.text("No risky implementations found.",c,l+12),l+=30):(w(e,{startY:l,margin:{left:c,right:c},head:[["#","Risk","Rule","Type","Library","Suggested component","Storybook"]],body:m.map(a=>[String(a.annotationId),String(a.severity||""),String(a.rule||""),String(a.componentType||""),String(a.libraryLabel||"—"),String(a.recommendedComponent||a.fluentAlternative||"n/a"),String(a.storybookUrl||"—")]),styles:{fontSize:7.5,cellPadding:3,valign:"top",overflow:"linebreak"},headStyles:{fillColor:[122,74,42]},columnStyles:{0:{cellWidth:22},1:{cellWidth:44},2:{cellWidth:110},3:{cellWidth:70},4:{cellWidth:90},5:{cellWidth:110},6:{cellWidth:266}}}),l=e.lastAutoTable.finalY+18),e.setFont("helvetica","bold"),e.setFontSize(14),e.text("Component inventory",c,l),l+=8,w(e,{startY:l,margin:{left:c,right:c},head:[["ID","Type","Library component","Props","Source","Risk","Conf.","Text","Selector","Storybook"]],body:p.map(a=>{const o=a?.detectedProps&&Object.keys(a.detectedProps).length?Object.entries(a.detectedProps).map(([b,g])=>`${b}=${g}`).join(" "):"—";return[String(a?.id||""),String(a?.componentType||""),String(a?.libraryComponentName||a?.fluentComponent||"—"),o,String(a?.possibleSource||""),String(a?.risk||""),`${a?.confidence??""}%`,String(a?.visibleText||"").substring(0,40),String(a?.selector||"").substring(0,80),String(a?.storybookUrl||"—")]}),styles:{fontSize:6.5,cellPadding:2,valign:"top",overflow:"linebreak"},headStyles:{fillColor:[40,40,40]},columnStyles:{0:{cellWidth:40},1:{cellWidth:54},2:{cellWidth:70},3:{cellWidth:90},4:{cellWidth:44},5:{cellWidth:36},6:{cellWidth:30},7:{cellWidth:96},8:{cellWidth:110},9:{cellWidth:142}}}),e.save(`${x}.pdf`)}function O(n,e){const i=e?.summary||{},m=C(e,10),p=Array.isArray(e?.inventory)?e.inventory:[],u=i.componentDistribution||{},x=m.length;let r=`
  ...
  `;const c=e?.fluentMeta||{},t=e?.notice,l=e?.librariesDetected||[],a=e?.frameworkMeta||{frameworks:[],generator:null};if(t&&(r+=`
      <div class="mb-6 rounded border border-ink/15 bg-cream/40 p-4 text-sm text-ink dark:bg-cream/10">
        <p class="m-0"><strong>Heads up:</strong> ${s(t)}</p>
      </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold uppercase tracking-widest text-muted">
                ${s((d.severity||"").toUpperCase())} RISK${d.priority?` · <span class="rounded bg-cream px-1.5 py-0.5 text-[10px] tracking-wider text-ink dark:bg-cream/20 dark:text-ink">${s(d.priority)}</span>`:""}
              </p>
  ...
  `,n.innerHTML=r,n.querySelectorAll("[data-ca-export-format]").forEach(o=>{o.addEventListener("click",()=>{const b=o.getAttribute("data-ca-export-format");if(b==="pdf")D(e);else if(b==="md"){const g=j(e),h=`component-audit-${new URL(e.url).hostname}-${new Date(e.timestamp||Date.now()).toISOString().slice(0,10)}`,d=new Blob([g],{type:"text/markdown;charset=utf-8"}),y=document.createElement("a");y.href=URL.createObjectURL(d),y.download=`${h}.md`,document.body.appendChild(y),y.click(),y.remove(),URL.revokeObjectURL(y.href)}})})}function N(){const n=document.getElementById("component-audit-tool"),e=n?.dataset.componentAuditApiUrl||"",i=["localhost","127.0.0.1","::1"].includes(window.location.hostname)||window.location.hostname.endsWith(".local"),m=window.__COMPONENT_AUDIT_API_URL||e||(i?"http://localhost:3000/api/component-audit":`${window.location.origin}/api/component-audit`),p=document.getElementById("component-audit-url"),u=document.getElementById("run-component-audit-btn"),x=document.getElementById("component-result-container");!n||!p||!u||!x||(window.location.hash==="#component-audit-tool"&&requestAnimationFrame(()=>{n.scrollIntoView({block:"start"})}),u.addEventListener("click",async()=>{const r=p.value.trim();if(!r){k(x,"Please enter a URL.");return}if(typeof navigator<"u"&&!navigator.onLine){k(x,"You appear to be offline.","Reconnect to the internet, then run the audit again.");return}try{new URL(r.startsWith("http")?r:"https://"+r)}catch{k(x,"Please enter a valid URL.");return}u.disabled=!0,u.textContent="Running audit...";const c=P(x);try{const t=new AbortController,l=window.setTimeout(()=>t.abort(),9e4),a=await fetch(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:r}),signal:t.signal});if(window.clearTimeout(l),!a.ok){let b=null;try{b=await a.json()}catch{b=null}const g=A(b?.error);k(x,g,b?.hint||U(a.status,g));return}const o=await a.json();O(x,o)}catch(t){const l=String(t?.message||"").toLowerCase(),a=t?.name==="AbortError"||l.includes("aborted");k(x,a?"This audit took too long to finish.":"Unable to contact the audit service.",a?"The service may be busy right now. Please retry in a minute.":"Check your connection and retry. If this persists, the API may be temporarily unavailable.")}finally{c.stop(),u.disabled=!1,u.textContent="Run audit"}}))}export{N as initializeComponentAuditTool};
