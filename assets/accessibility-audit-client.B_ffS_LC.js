import{E as I,a as A}from"./jspdf.plugin.autotable.BNWpDLH5.js";import"./preload-helper.BlTxHScW.js";function r(n){return String(n).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function v(n,t,i){const u=r(t),d=i?r(i):"";n.innerHTML=`
  ...
  `}function L(n,t){const i=String(t||"").toLowerCase();return n===429?"The service is currently busy. Please retry in about 30-60 seconds.":n===502||n===503||n===504?"The audit service is temporarily unavailable or under maintenance. Please try again shortly.":n===408?"The page took too long to load. Retry once, then test a lighter page to confirm connectivity.":n===422||i.includes("could not reach")||i.includes("could not be resolved")?"Verify that the page is public, not login-gated, and not blocking automated browsers.":"Please try again in a moment. If this keeps happening, the audit service may be temporarily unavailable."}function w(n){const t=Math.floor(n/1e3);return`${Math.floor(t/60)}:${String(t%60).padStart(2,"0")}`}function C(n){const t=["Connecting to M.ai audit service","Opening page in a secure browser session","Running accessibility checks and collecting evidence","Packaging your report"],i=Date.now();let u=0;const d=()=>{const e=w(Date.now()-i),m=t.map((a,p)=>{const o=p===u;return`<li class="flex items-center gap-2 text-sm ${o?"text-ink font-semibold":"text-muted"}"><span class="inline-block h-2 w-2 rounded-full ${o?"bg-accent animate-pulse":"bg-ink/20"}" aria-hidden="true"></span>${r(a)}</li>`}).join("");n.innerHTML=`
    ...
    `};d();const s=window.setInterval(d,1e3);return{elapsed(){return Date.now()-i},stop(){window.clearInterval(s)}}}function R(n){const t=String(n||"").trim();if(!t)return"The audit request failed.";const i=t.replace(/^audit failed:\s*/i,""),d=i.split(/browser logs?:/i)[0].trim()||i,s=d.toLowerCase();return s.includes("target page, context or browser has been closed")?"This site could not be audited because the browser session closed unexpectedly.":s.includes("timeout")||s.includes("timed out")?"The audit timed out before the page finished loading.":s.includes("err_name_not_resolved")||s.includes("dns")?"The URL could not be resolved.":s.includes("net::err_")||s.includes("failed to fetch")?"The audit could not reach that URL.":d.length>220?`${d.slice(0,217)}...`:d}function T(n,t=10){return(Array.isArray(n?.findings)?n.findings:[]).slice(0,t).map((u,d)=>({...u,annotationId:d+1}))}function $(n){return n?`${n.x}, ${n.y}, ${n.width} × ${n.height}`:""}function M(n){const t=n?.summary||{},i=T(n,10),u=Array.isArray(n?.inventory)?n.inventory:[],d=n?.screenshot,s=["# Accessibility Audit","",`- URL: ${n?.url||""}`,`- Timestamp: ${n?.timestamp||""}`,`- Elements: ${t.totalElements??0}`,`- Findings: ${t.totalFindings??0}`,`- Critical: ${t.critical??0}`,`- Serious: ${t.serious??0}`,`- Moderate: ${t.moderate??0}`,`- Minor: ${t.minor??0}`,"","## Method","","- Accessibility Annotation Playbook: capture intent, label states, and preserve focus order.","- Accessibility Compliance Baseline Playbook: validate contrast, reflow, focus, and shell primitives.","- Accessibility Audit Ops Playbook: keep the scan, evidence, and follow-up in one repeatable format.","","## Annotations",""];return i.length===0?s.push("No annotated findings to review.",""):i.forEach(e=>{s.push(`### Annotation ${e.annotationId} - ${e?.rule||"Finding"}`,"",`- Severity: ${e?.severity||""}`,`- Element: ${e?.elementId||""}`,`- Selector: ${e?.selector||""}`,`- Bounding box: ${$(e?.boundingBox)}`,`- Why it matters: ${e?.whyItMatters||""}`,`- Suggested fix: ${e?.suggestedFix||""}`,"")}),s.push("## Findings"),i.length===0?s.push("","No issues found."):(s.push("","| Annotation | Severity | Rule | Element | Selector | BBox |","|---|---|---|---|---|---|"),i.forEach(e=>{s.push(`| ${e.annotationId} | ${e?.severity||""} | ${e?.rule||"Finding"} | ${e?.elementId||""} | ${e?.selector||""} | ${$(e?.boundingBox)} |`)})),s.push("","## Screenshot",""),d?.data?s.push(`![Annotated page screenshot](data:${d.mimeType||"image/png"};base64,${d.data})`):s.push("Screenshot unavailable."),s.push("","## Element Inventory","","| ID | Type | Text | Role | Selector |","|---|---|---|---|---|"),u.slice(0,20).forEach(e=>{s.push(`| ${e?.id||""} | ${e?.type||""} | ${e?.text||""} | ${e?.role||""} | ${e?.selector||""} |`)}),s.push("","## M.ai Context","","```json",JSON.stringify(n?.maiContext||{},null,2),"```"),s.join(`
`)}function P(n){const t=new I({orientation:"portrait",unit:"pt",format:"letter"}),i=n?.summary||{},u=T(n,10),d=Array.isArray(n?.inventory)?n.inventory:[],s=n?.screenshot,e=`accessibility-audit-${new URL(n.url).hostname}-${new Date(n.timestamp||Date.now()).toISOString().slice(0,10)}`,m=t.internal.pageSize.getWidth(),a=40,p=m-a*2;let o=48;if(t.setFont("helvetica","bold"),t.setFontSize(18),t.text("Accessibility Audit by M.ai",a,o),o+=18,t.setFont("helvetica","normal"),t.setFontSize(10),t.text(`URL: ${n.url}`,a,o),o+=14,t.text(`Timestamp: ${n.timestamp||""}`,a,o),o+=18,s?.data){t.setFont("helvetica","bold"),t.setFontSize(14),t.text("Annotated screenshot",a,o),o+=10;const l=Number(s.width||1),h=Number(s.height||1),g=Math.max(220,Math.round(p*(h/l)));t.addImage(`data:${s.mimeType||"image/png"};base64,${s.data}`,"PNG",a,o,p,g);const y=p/l,f=g/h;t.setDrawColor(204,51,51),t.setLineWidth(1),u.forEach(x=>{if(!x?.boundingBox)return;const k=a+x.boundingBox.x*y,S=o+x.boundingBox.y*f,F=x.boundingBox.width*y,E=x.boundingBox.height*f;F>0&&E>0&&(t.rect(k,S,F,E),t.setFillColor(122,74,42),t.circle(k+8,S+8,8,"F"),t.setTextColor(255,255,255),t.setFont("helvetica","bold"),t.setFontSize(8),t.text(String(x.annotationId),k+8,S+10.5,{align:"center"}),t.setTextColor(0,0,0))}),o+=g+18}A(t,{startY:o,margin:{left:a,right:a},head:[["Elements","Findings","Critical","Serious","Moderate","Minor"]],body:[[String(i.totalElements??0),String(i.totalFindings??0),String(i.critical??0),String(i.serious??0),String(i.moderate??0),String(i.minor??0)]],styles:{fontSize:9,cellPadding:4},headStyles:{fillColor:[40,40,40]}}),o=t.lastAutoTable.finalY+18,t.setFont("helvetica","bold"),t.setFontSize(14),t.text("Annotations",a,o),o+=8,u.length===0?(t.setFont("helvetica","normal"),t.setFontSize(10),t.text("No issues found.",a,o+12)):A(t,{startY:o,margin:{left:a,right:a},head:[["Annotation","Severity","Rule","Element","Selector","BBox"]],body:u.map(l=>[String(l.annotationId),String(l?.severity||""),String(l?.rule||""),String(l?.elementId||""),String(l?.selector||""),$(l?.boundingBox)]),styles:{fontSize:8,cellPadding:3,valign:"top"},headStyles:{fillColor:[122,74,42]},columnStyles:{0:{cellWidth:24},1:{cellWidth:52},2:{cellWidth:120},3:{cellWidth:54},4:{cellWidth:170},5:{cellWidth:88}}}),o=t.lastAutoTable?t.lastAutoTable.finalY+18:o+32,t.setFont("helvetica","bold"),t.setFontSize(14),t.text("Element inventory",a,o),o+=8,d.length>0&&A(t,{startY:o,margin:{left:a,right:a},head:[["ID","Type","Text","Role","Selector"]],body:d.slice(0,20).map(l=>[String(l?.id||""),String(l?.type||""),String(l?.text||""),String(l?.role||""),String(l?.selector||"")]),styles:{fontSize:8,cellPadding:3,valign:"top"},headStyles:{fillColor:[40,40,40]},columnStyles:{0:{cellWidth:70},1:{cellWidth:48},2:{cellWidth:150},3:{cellWidth:56},4:{cellWidth:192}}}),o=t.lastAutoTable?t.lastAutoTable.finalY+18:o+32,t.setFont("helvetica","bold"),t.setFontSize(14),t.text("M.ai context",a,o),o+=8,t.setFont("helvetica","normal"),t.setFontSize(9);const c=JSON.stringify(n?.maiContext||{},null,2),b=t.splitTextToSize(c,m-a*2);t.text(b,a,o+12),t.save(`${e}.pdf`)}function B(n,t){const i=t?.summary||{},u=T(t,10),d=Array.isArray(t?.inventory)?t.inventory:[];let s=`
  ...
  `,t?.screenshot?.data){const e=t.screenshot,m=Number(e.width||1),a=Number(e.height||1),p=u.filter(o=>o?.boundingBox).map(o=>{const c=o.boundingBox,b=c.x/m*100,l=c.y/a*100,h=c.width/m*100,g=c.height/a*100;return`
          <div class="absolute border-2 border-red-600/90 bg-red-600/10" title="${r(o.rule||"")}" style="left:${b}%; top:${l}%; width:${h}%; height:${g}%;"></div>
          <div class="absolute z-10" style="left:${b}%; top:${l}%; transform: translate(-50%, -50%);">
    ...
    `}else s+='<p class="mt-5 rounded bg-emerald-50 p-4 text-emerald-800">No issues found.</p>';s+=`
      <div class="mt-6 overflow-x-auto">
        <table class="min-w-full border-collapse text-left text-sm">
          ...
          <tbody>
  `,u.forEach(e=>{const m=e?.elementId||"n/a",a=e?.selector||"n/a";s+=`
      <tr class="border-b border-ink/5 align-top last:border-b-0">
        ...
        <td class="py-3 pr-3 text-muted">${r($(e?.boundingBox))}</td>
      </tr>
    `}),s+="</tbody></table></div>",u.length>10&&(s+=`<p class="mt-3 text-xs text-muted">... and ${r(u.length-10)} more annotated findings</p>`),s+=`
  ...
  `,d.slice(0,8).forEach(e=>{s+=`
    ...
    `}),s+="</tbody></table></div>",d.length>8&&(s+=`<p class="mt-3 text-xs text-muted">... and ${r(d.length-8)} more elements</p>`),s+="</details>",s+=`
  ...
  `,n.innerHTML=s,n.querySelectorAll("[data-export-format]").forEach(e=>{e.addEventListener("click",()=>{const m=e.getAttribute("data-export-format");if(m==="pdf")P(t);else if(m==="md"){const a=M(t),p=`accessibility-audit-${new URL(t.url).hostname}-${new Date(t.timestamp||Date.now()).toISOString().slice(0,10)}`,o=new Blob([a],{type:"text/markdown;charset=utf-8"}),c=document.createElement("a");c.href=URL.createObjectURL(o),c.download=`${p}.md`,document.body.appendChild(c),c.click(),c.remove(),URL.revokeObjectURL(c.href)}})})}function z(){const n=document.getElementById("ask-mai"),t=n?.dataset.auditApiUrl||"",i=["localhost","127.0.0.1","::1"].includes(window.location.hostname)||window.location.hostname.endsWith(".local"),u=window.__AUDIT_API_URL||t||(i?"http://localhost:3000/api/audit":`${window.location.origin}/api/audit`),d=document.getElementById("audit-url"),s=document.getElementById("run-audit-btn"),e=document.getElementById("result-container");if(!n||!d||!s||!e)return;window.location.hash==="#ask-mai"&&requestAnimationFrame(()=>{n.scrollIntoView({block:"start"})});let m=!1;d.addEventListener("focus",()=>{m||(m=!0,window.dispatchEvent(new CustomEvent("site:track",{detail:{name:"tool_engage",params:{tool:"accessibility"}}})))},{once:!1}),s.addEventListener("click",async()=>{const a=d.value.trim();if(!a){v(e,"Please enter a URL.");return}if(typeof navigator<"u"&&!navigator.onLine){v(e,"You appear to be offline.","Reconnect to the internet, then run the audit again.");return}try{new URL(a.startsWith("http")?a:"https://"+a)}catch{v(e,"Please enter a valid URL.");return}s.disabled=!0,s.textContent="Running audit...";const p=C(e),o=(c,b)=>window.dispatchEvent(new CustomEvent("site:track",{detail:{name:c,params:b}}));o("audit_run",{tool:"accessibility"});try{const c=new AbortController,b=window.setTimeout(()=>c.abort(),9e4),l=await fetch(u,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:a}),signal:c.signal});if(window.clearTimeout(b),!l.ok){let y=null;try{y=await l.json()}catch{y=null}const f=R(y?.error),x=w(p.elapsed());o("audit_error",{tool:"accessibility",http_status:l.status}),v(e,f,`${y?.hint||L(l.status,f)} (after ${x})`);return}const h=await l.json(),g=p.elapsed();o("audit_complete",{tool:"accessibility",finding_count:h?.summary?.totalFindings??null,elapsed_ms:g}),B(e,{...h,_elapsedMs:g})}catch(c){const b=String(c?.message||"").toLowerCase(),l=c?.name==="AbortError"||b.includes("aborted"),h=w(p.elapsed());o("audit_error",{tool:"accessibility",reason:l?"timeout":"network"}),v(e,l?"This audit took too long to finish.":"Unable to contact the audit service.",l?`The service may be busy right now. Please retry in a minute. (after ${h})`:`Check your connection and retry. If this persists, the API may be temporarily unavailable. (after ${h})`)}finally{p.stop(),s.disabled=!1,s.textContent="Start audit"}})}export{z as initializeAccessibilityAuditTool};
